...
Цена: {product[3]}''', reply_markup=generate_product_detail_menu(product_id=product_id, category_id=product[1],
                                                                 cart_id=cart_id, product_name=product[2]))
...
async def add_product_cart(call: CallbackQuery):
    ...
    cart_id = get_user_cart_id(chat_id)
    message_id = call.message.message_id
    _, znak, product_id, count = call.data.split('_')  # cart_1_2
    product_id, count = int(product_id), int(count)
...
Цена: {product[3]}''', reply_markup=generate_product_detail_menu(product_id=product_id, category_id=product[1],
        ...
        delete_product(cart_id, product_name=product[2])
        await bot.answer_callback_query(call.id, 'Нельзя ниже нуля', )
        await bot.edit_message_caption(chat_id=chat_id, message_id=message_id, caption=f'''{product[2]}
